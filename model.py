...
class LSTM_Model(nn.Module):
    def __init__(self, num_classes, middle_layer_shape, hidden_size, device):
        ...
        self.lstm = nn.LSTM(middle_layer_shape + 1, hidden_size)
        self.out = nn.Linear(hidden_size, num_classes)
        self.device = device
        self.dropout = nn.Dropout(0.5)
    def forward(self, input, attention,forecast_prediction):
        maximum_length = input.size(1)
        cumulative_lstm_input = []
        for i in range(maximum_length):
            forecast_information = self.Forecast(input[:, i, :], attention[:, i, :])
            lstm_input = torch.cat((forecast_information, forecast_prediction[:, i].unsqueeze(1)), dim=-1)
            cumulative_lstm_input.append(lstm_input.tolist())
        cumulative_lstm_input = torch.tensor(cumulative_lstm_input, dtype=torch.float).to(self.device)
